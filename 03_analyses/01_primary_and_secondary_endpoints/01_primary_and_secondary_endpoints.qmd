...
### Alternative outcome: relative abundance of *L. crispatus*


```{r}

L_crisp_rel_ab <- 
  get_assay_long_format(mae, "prop_Lacto") |> 
  filter(feature == "prop_L_crisp", AVISITN %in% c(4, 7)) |> 
  mutate(
    L_crisp_rel_ab = value,
    Visit = AVISITN |> get_visit_labels()
    ) |> 
  select(USUBJID, ARM, Visit, L_crisp_rel_ab)

L_crisp_rel_ab_summary <- 
  L_crisp_rel_ab |> 
  group_by(Visit, ARM) |> 
  summarize(
    n = n(),
    mean = mean(L_crisp_rel_ab),
    sd_rel_ab = sd(L_crisp_rel_ab, na.rm = TRUE),
    se = sd_rel_ab / sqrt(n), 
    ci_lower = pmax(0, mean - qt(0.975, df = pmax(n - 1, 1)) * se),
    ci_upper = pmin(1, mean + qt(0.975, df = pmax(n - 1, 1)) * se),
    .groups = "drop"
    )
...
g_continuous <- 
  L_crisp_rel_ab |> 
  ggplot() +
  aes(x = ARM, y = L_crisp_rel_ab, col = ARM, fill = ARM) +
  facet_grid(. ~ Visit) +
  geom_violin(bw = 0.075, bounds = c(0, 1), scale = "width", alpha = 0.3, linewidth = 0.2, color = "transparent") + # 
  ggbeeswarm::geom_quasirandom(size = 1, alpha = 0.5) +
  geom_linerange(
    data = L_crisp_rel_ab_summary, 
    aes(x = ARM, y = mean, ymin = ci_lower, ymax = ci_upper), 
    col = "black", linewidth = 1, alpha = 0.6, lineend = "round"
  ) +
  geom_point(
    data = L_crisp_rel_ab_summary, 
    aes(x = ARM, y = mean), col = "black", size = 3, shape = 18 
  ) +
  scale_y_continuous(
    expression(paste(italic("L. crisp."), " rel. ab.")),
...
L_crisp_rel_ab_summary |> 
  group_by(Visit) |> 
  mutate(
    ci = str_c(ci_lower |> scales::percent(accuracy = 0.1), " - ", ci_upper |> scales::percent(accuracy = 0.1)),
    mean = mean |> scales::percent(accuracy = 0.1)
  ) |> 
  select(-sd_rel_ab, -se, -ci_lower, -ci_upper) |>
  gt(
    caption = "Mean relative abundance of L. crispatus (and 95% CI) at week 12 and week 24 in both arms",
...
L_crisp_rel_ab |> 
  ...
  summarize(
    n = n(),
    n_in_middle = sum((L_crisp_rel_ab > 1/3) & (L_crisp_rel_ab < 2/3)),
  ) |> 
  ungroup() |> 
  mutate(perc_in_middle = n_in_middle/n) |> 
  gt(
    caption = "Number and percentage of participants with relative abundance of L. crispatus between 33% (1/3) and 67% (2/3) at week 12 and week 24",
    row_group_as_column = TRUE
  ) |> 
  cols_label(
    Visit = "Visit",
    n = "N",
    n_in_middle = "N (33% < rel. ab. < 67%)",
    perc_in_middle = "Perc. (33% < rel. ab. < 67%)"
  ) |> 
  fmt_percent(
    columns = perc_in_middle,
    decimals = 2
  )
...
map(
  .x = seq(1/3, 2/3, by = 1/12),
  .f = function(thresh){
    L_crisp_rel_ab |> 
      mutate(outcome = str_c("L. crisp. ", ifelse(L_crisp_rel_ab >= thresh, "≥", "<")," ", scales::percent(thresh, accuracy = 1))) |> 
      dplyr::count(Visit, ARM, outcome) |> 
      arrange(Visit, ARM, outcome) |> 
      mutate(thresh = scales::percent(thresh, accuracy = 1))
  } 
) |> 
  bind_rows() |>
  select(thresh, everything()) |> 
  arrange(thresh, Visit, outcome, ARM |> desc()) |> 
  group_by(thresh, Visit) |> 
  mutate(
    BR = epitools::riskratio(x = matrix(n, nrow = 2))$measure[2,1],
    CI_low = epitools::riskratio(x = matrix(n, nrow = 2))$measure[2,2],
    CI_high = epitools::riskratio(x = matrix(n, nrow = 2))$measure[2,3],
    Benefit_Ratio = str_c(BR |> round(2), " (", CI_low |> round(2), " - ", CI_high |> round(2), ")")
  ) |> 
  select(-BR, -CI_low, -CI_high) |> 
  pivot_wider(names_from = ARM, values_from = n, names_prefix = "N ") |>
  relocate(Benefit_Ratio, .after = last_col()) |>
  mutate(
    Benefit_Ratio = ifelse(str_detect(outcome, "≥"), Benefit_Ratio, "")
    ) |> 
  group_by(thresh) |> 
  gt(
    row_group_as_column = TRUE,
    caption = "Benefit ratios (and 95% CI) for different thresholds of L. crispatus relative abundance at week 12 and week 24. 
    Thresholds range from 33% to 67% in increments of 8.33%, representing evenly distributed cut-offs"
  )

  

# and well-separated by our pre-specified 50% threshold
...
category_labels <- 
  c(sankey_data$endpoint |> levels(), "") |> 
  str_replace(", < 50% L. crispatus", ", < 50% *L.c.*") |> 
...
tmp <- 
  ...
  mutate(
    topic_print_label = 
      str_c("*", topic_label, "*") |> 
      str_replace("jensenii/mulieres", "jens./mul.") |>
      str_replace(" topic\\*", "* topic") |> 
      str_replace("\\*Other ", "Other *") |> 
      str_replace(" L\\. v\\. \\(BVAB1\\)\\*", "* L. v. (BVAB1)") |> 
      fct_inorder()
  )

# tmp |> dplyr::count(topic_label, topic_print_label)
...
#   expression(italic("L. jens./mul.")),
#   expression(Other~italic("L.")), 
#   expression(italic("Gardnerella")~"topic"),
#   expression(italic("Ca.")~"L. v. (BVAB1) topic"),
#   expression(italic("P. bivia")~"topic"),
#   expression(italic("P. timonensis")~"topic")
# )
...
g_subcommunities_traj <- 
  ...
  scale_fill_manual(
    ...
    breaks = tmp$topic_label |> levels(), 
    labels = tmp$topic_print_label |> levels()
  ) +
...
g_S2D <- g_violin 
save(g_S2D, file = str_c(fig_out_dir(), "supplementary_ggplots/S2D.Rdata"))
...
g_spaghetti <- 
  ...
  facet_grid(Arm ~ topic_print_label |> str_replace("topic", "<br>topic") |> str_replace("\\(BVAB1\\) <br>topic", "<br>\\(BVAB1\\) topic") |> fct_inorder()) +
...
BC_between_visits_with_endpoints |> 
  filter(v1 >= 2, ARM == "LACTIN-V") |> 
  mutate(
    mb_categories = 
      ifelse(mb_categories_v1 == "≥ 50% L. crispatus", "≥ 50% L. crispatus", "< 50% L. crispatus") |> 
      factor()
  ) |> 
  group_by(ARM, mb_categories, v1_label, v2_label) |> 
  summarize(
    median_BC = median(BC),
    q25 = quantile(BC, p = 0.25),
    q75 = quantile(BC, p = 0.75),
    median_and_IQR = str_c(
      median_BC |> format(digits = 2), 
      " (", q25 |> format(digits = 2), " - ", q75 |> format(digits = 2), ")"
    ),
    mean_BC = mean(BC),
    ci_lower = pmax(0, t.test(BC)$conf.int[1]),
    ci_upper = pmin(1, t.test(BC)$conf.int[2]),
    mean_and_CI = str_c(
      mean_BC |> format(digits = 2), 
      " (", ci_lower |> format(digits = 2), " - ", ci_upper |> format(digits = 2), ")"
    ),
    n = n(),
    .groups = "drop"
  ) |>  
  mutate(
    visits = str_c(v1_label, " to ", v2_label) |> fct_inorder()
   ) |> 
  select(
    visits, mb_categories, n,
    median_and_IQR, mean_and_CI
  ) |>
  arrange(visits) |>
  group_by(visits) |> 
  gt(
    caption = "Median (IQR) and mean (95% CI) Bray-Curtis dissimilarities between samples at consecutive visits for participants in the Lactin-V arm, stratified by microbiota category at the first of the two consecutive visits.",
    row_group_as_column = TRUE
  ) |> 
  cols_label(
    mb_categories = "Microbiota category",
    n = "N",
    median_and_IQR = "Median (IQR) BC",
    mean_and_CI = "Mean (95% CI) BC"
  )
...
g <- 
  ...
  geom_violin(col = "white", scale = "width", alpha = 0.5, drop = TRUE, draw_quantiles = c(0.25, 0.5, 0.75)) +
...
g_S2E <- g
save(g_S2E, file = str_c(fig_out_dir(), "supplementary_ggplots/S2E.Rdata"))
...
p_week12_comp <- 
  ...
  (
    g_12_P + 
      theme(strip.text.x = element_markdown(color = "transparent")) +  
...
fig_1 <- 
  free(g_study_timeline) + # a 
  free(g_sankey_v2 + theme(legend.position = "bottom")) + # b # xlab("") +
  p_week12_comp + # c
  g_continuous + # d
  g_load_v1 + # e
  plot_layout(
...
ggsave(fig_1, filename = str_c(fig_out_dir(), "Fig1.pdf"), width = 6.5, height = 5, scale = 2, device = cairo_pdf)
