...
g_shannon <- 
  df_load |> 
  filter(!is.na(shannon_v0)) |> 
  ggplot() +
  aes(y = RACEGR2, x = shannon_v0, fill = RACEGR2) +
   geom_boxplot(alpha = 0.5, outlier.shape = NULL) +
  geom_point(alpha = 0.5) +
  labs(
    y = "Self-declared race",
...
g_race_edu <- 
  ...
  arrange(EDULVL) |> 
  ggplot() +
  aes(y = RACEGR2 |> fct_rev(), x = EDULVL |> str_wrap(width = 15) |> fct_inorder(), fill = RACEGR2) +
  geom_tile(aes(alpha = n)) +
  geom_text(aes(label = n)) + 
  # scale_alpha(range = c(0,1)) +
  # scale_fill_gradient(low = "white", high = "steelblue") +
  ylab("Self-declared\nparticipants' race") + xlab("Education\n") +
  scale_fill_manual(values = c("purple1", "darkseagreen3", "gold3")) +
  guides(fill = "none", alpha = "none") +
  theme(
    # axis.text.x = element_text(angle = 45, hjust = 1),
    axis.title.y = element_text(angle = 0, hjust = 1, vjust = 1, margin = margin(r = -40))
  )
...
g_shannon <- 
  df_shannon_v0 |> 
  ggplot() +
  aes(y = RACEGR2 |> fct_rev(), x = shannon_v0, fill = RACEGR2) +
  geom_violin(alpha = 0.5, draw_quantiles = 0.5, color = "white", linewidth = 1) +
  # geom_point(alpha = 0.5) +
  ggbeeswarm::geom_quasirandom(width = 0.3, size = 0.5, aes(col = RACEGR2)) +
  labs(
...
g_race_by_site <- 
  ...
  as_tibble() |> 
  filter(!is.na(ARM)) |> 
  select(USUBJID, ARM, SITENAME, RACEGR2) |> 
  distinct() |> 
  ...
  ungroup() |> 
  ggplot() +
  aes(x = SITENAME |> str_wrap(width = 14), y = RACEGR2 |> fct_rev(), fill = RACEGR2) +
  ...
  guides(fill = "none", alpha = "none") +
  theme(
    axis.title.y = element_text(angle = 0, hjust = 1, vjust = 1, margin = margin(r = -60))
  )
...
g <- 
  g_race_edu + labs(tag = LETTERS[i]) + 
  g_shannon + labs(tag = LETTERS[i + 1]) + ylab("") + theme(axis.text.y = element_blank()) +
  g_race_by_site + labs(tag = LETTERS[i + 2]) + ylab("") + theme(axis.text.y = element_blank()) +
...
g_S7B <- g_race_edu
g_S7C <- g_shannon
g_S7D <- g_race_by_site

save(g_S7B, file = str_c(fig_out_dir(), "supplementary_ggplots/S7B.Rdata"))
save(g_S7C, file = str_c(fig_out_dir(), "supplementary_ggplots/S7C.Rdata"))
save(g_S7D, file = str_c(fig_out_dir(), "supplementary_ggplots/S7D.Rdata"))
