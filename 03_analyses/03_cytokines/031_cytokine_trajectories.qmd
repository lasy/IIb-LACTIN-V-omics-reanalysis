...
t_test_res <- 
  map(
    cytokine_names,
    effects_of_intervention_student,
    cytokines_to_preMTZ = cytokines_to_preMTZ
  ) |> 
  bind_rows() |> 
  mutate(
...
bind_rows(
  res |> mutate(test = "Wilcoxon signed rank test"),
  t_test_res |> mutate(test = "Student t-test")
  ) |> 
  ggplot() +
  aes(x = cytokine, y = estimate, col = test) +
  geom_hline(yintercept = 0, col = "gray", linewidth = 1) +
  geom_linerange(aes(ymin = CI_lower, ymax = CI_upper), position = position_dodge(width = 0.3)) +
  geom_point(size = 2, position = position_dodge(width = 0.3)) +
  xlab("") +
  ylab("Estimated difference in location (Lactin-V - Placebo)\nand associated 95% CI.") +
  labs(caption = "Test on the difference between log10(conc.) at baseline (pre-MTZ) and Week 24 visit") +
  theme(
...
#| cache: false
g_S4D <- g
save(g_S4D, file = str_c(fig_out_dir(), "supplementary_ggplots/S4D.Rdata"))
...
w_test_res <- 
  ...
  filter(cytokine %in% selected_cytokines) |> 
  mutate(
    Visit = "Week 24" |> factor(),
    xmin = 1, xmax = 2,
    group1 = "LACTIN-V", group2 = "Placebo",
    label = case_when(
      adj_p_value < 0.001 ~ "***",
      adj_p_value < 0.01 ~ "**",
      adj_p_value < 0.05 ~ "*",
  ...
  left_join(
    g_selection@data |> 
      ...
      group_by(cytokine) |>
      summarise(y.position = max(value) + 0.5)
  )

g_selection <- 
  g_selection +
  ggpubr::stat_pvalue_manual(data = w_test_res)
...
save(g_selection, w_test_res, file = str_c(data_dir(), "05_figures_tables/fig_3a_PC1_subtracted_all.Rdata"))
