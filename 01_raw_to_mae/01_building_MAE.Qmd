...
tmp |> 
  ...
  gt::gt(
    caption = "Number of samples with 16S rRNA data per arm",
    row_group_as_column = TRUE
  ) |> 
  gt::summary_rows(
...
tmp |> 
  ...
  gt::gt(
    caption = "Number of unique study visits with 16S rRNA data per arm",
    row_group_as_column = TRUE
    ) |> 
  gt::summary_rows(
...
Specifically, we check and change the ASV taxonomic assignments for *Gardnerella* spp. and *Lactobacillus jensenii/mulieres* 
...
ps@tax_table |> 
  ...
  # as_tibble() |> 
  dplyr::filter(
    (Genus == "Gardnerella" & Species != "") | 
      (Genus == "Lactobacillus" & Species %in% c("jensenii", "mulieris"))
  )

```

We change the species assignment for these ASVs as follows:
...
tax_table(ps) <-
  ...
  mutate(
    ...
    Species = 
      case_when(
        (Genus == "Lactobacillus") & (Species == "jensenii") ~ "jensenii/mulieres",
        (Species_original == "swidsinskii_leopoldii") & (Genus == "Gardnerella") & (row_number() == 3) ~ "(ASV G1)",
        (Species_original == "vaginalis") & (Genus == "Gardnerella") & (row_number() == 6) ~ "(ASV G2)",
        (Species_original == "piotii") & (Genus == "Gardnerella") & (row_number() == 16) ~ "(ASV G3)",
        (Genus == "Gardnerella") ~ "",
        TRUE ~ Species
      )
  ) |> 
  select(Kingdom, Phylum, Class, Order, Family, Genus, Species, Species_original, Gen_Spec) |>
  as.matrix() |> 
  tax_table()
...
ps@tax_table |> 
  ...
  dplyr::filter(
    (Genus == "Gardnerella" & Species_original != "") | 
      (Genus == "Lactobacillus" & Species_original %in% c("jensenii", "mulieris"))
  ) |> 
  arrange(Genus, ASV_number, Species) |> 
  select(Family, Genus, Species, Species_original, ASV_number) |> 
  gt::gt(caption = "Subset of the taxonomic table highlighting taxonomic assignment changes.")
...
tmp |> 
  dplyr::count(Arm, name = "n samples with 16S data") |> 
  gt::gt(caption = "Number of samples with 16S rRNA data per arm after filtering") 
...
tmp |> 
  dplyr::count(Arm, USUBJID, AVISITN) |> 
  dplyr::count(Arm, name = "n study visits with 16S data") |>
  gt::gt(caption = "Number of unique study visits with 16S rRNA data per arm after filtering")
...
```{r}
# Exporting the taxonomic table

spreadsheet_output_dir <- fig_out_dir() |> str_c("spreadsheets/")
if (!fs::dir_exists(spreadsheet_output_dir)) {
  fs::dir_create(spreadsheet_output_dir)
}

tax_table(ps_included) |> 
  as.data.frame() |>
  select(-Species_original) |> 
  rownames_to_column(var = "ASV_sequence") |> 
  writexl::write_xlsx(
    path = str_c(spreadsheet_output_dir,"ASV_taxonomic_table.xlsx")
  )
...
ps_agg <- phyloseq::tax_glom(ps_included, taxrank = "Species", NArm = FALSE)
...
mae_sample_map <- 
  ...
  dplyr::rename(Barcode = primary) |>
  dplyr::select(Barcode, assay) |> 
  arrange(Barcode) |> 
...
all_shipped_swabs <- 
  colData |> 
  as_tibble() |> 
  dplyr::select(Barcode, USUBJID, ARM, AVISITN, PIPV, DAY) |>
