...
compare_coefs <- function(coefs){

  ggplot(coefs) +
    aes(y = variable |> fct_rev(), x = value, col = model, 
        alpha = CI_excludes_0) + 
    facet_grid(block ~ Yvar, scales = "free_y", 
               space = "free_y") +
...
coefs <-
  bind_rows(
    get_mtb_coef(mod_week4$res, boot = mod_week4$boot) |> 
      mutate(model = "Full"),
    get_mtb_coef(mod_week4_ext$res, boot = mod_week4_ext$boot) |> 
      mutate(model = "Extended"),
    get_mtb_coef(mod_week4_red$res, boot = mod_week4_red$boot) |> 
      mutate(model = "Reduced"),
    get_mtb_coef(mod_week4_min$res, boot = mod_week4_min$boot) |> 
      mutate(model = "Minimal")
  ) |> 
  mutate(
    model = model |> fct_inorder(),
    CI_excludes_0 = if_else(lo * up > 0, 1, 0),
    block = block |> fct_inorder()
    )

compare_coefs(coefs) +
  ggtitle("Initial phase (post-MTZ to week 4)")
...
coefs <-
  bind_rows(
    get_mtb_coef(mod_week8_and_12$res, boot = mod_week8_and_12$boot) |> 
      mutate(model = "Full"),
    get_mtb_coef(mod_week8_and_12_ext$res, boot = mod_week8_and_12_ext$boot) |> 
      mutate(model = "Extended"),
    get_mtb_coef(mod_week8_and_12_red$res, boot = mod_week8_and_12_red$boot) |> 
      mutate(model = "Reduced"),
    get_mtb_coef(mod_week8_and_12_min$res, boot = mod_week8_and_12_min$boot) |> 
      mutate(model = "Minimal")
  ) |> 
  mutate(
    model = model |> fct_inorder(),
...
compare_coefs(coefs) +
  ggtitle("Continuation phase (week 4 to week 8 to week 12)")
...
coefs <-
  bind_rows(
    get_mtb_coef(mod_week24$res, boot = mod_week24$boot) |> 
      mutate(model = "Full"),
    get_mtb_coef(mod_week24_ext$res, boot = mod_week24_ext$boot) |> 
      mutate(model = "Extended"),
    get_mtb_coef(mod_week24_red$res, boot = mod_week24_red$boot) |> 
      mutate(model = "Reduced"),
    get_mtb_coef(mod_week24_min$res, boot = mod_week24_min$boot) |> 
      mutate(model = "Minimal")
  ) |> 
  mutate(
    model = model |> fct_inorder(),
...
compare_coefs(coefs) +
  ggtitle("Follow-up phase (week 12 to week 24)")
...
df |> 
  ggplot(aes(x = `Gardnerella topic (pre-MTZ)`, y = pmax(`Any condomless sex`, `Any condom sex`))) +
  geom_jitter(height = 0.1, size = 0.5) +
  geom_smooth(method = "lm") 
...
df |> 
  ggplot(aes(x = `P. timonensis topic (pre-MTZ)`, y = pmax(`Any condomless sex`, `Any condom sex`))) +
  geom_jitter(height = 0.1, size = 0.5) +
  geom_smooth(method = "lm") 
...
### Sensitivity analysis by sites



Re-fitting the full models in each trial phase, including "study site" as a covariate in the demographics block.
...
model_par$input_blocks <- mod_week4$inputs_raw |> names()
model_par$input_blocks <- model_par$input_blocks |> str_replace("Demographics", "Demographics with site")
model_par$model_name <- mod_week4$model_name |> str_c(" site included in demographics")

mod_res4_sites <- perform_MBPLSDA_successive(model_par = model_par, mbplsda_par = mbplsda_par, IO_par = IO_par)
...
coefs <-
  bind_rows(
    get_mtb_coef(mod_res4_sites$res, boot = mod_res4_sites$boot) |> 
      mutate(model = "With site", block = block |> str_remove(" with site")),
    get_mtb_coef(mod_week4$res, boot = mod_week4$boot) |> 
...
plot_mtb_cov(mod_week4$res, scale_axes = "fixed") + 
  scale_color_manual(values = block_colors) + 
  ggtitle("post-MTZ to week 4 - without sites") + 
plot_mtb_cov(mod_res4_sites$res, scale_axes = "fixed") + 
  scale_color_manual(values = block_colors) + 
  ggtitle("post-MTZ to week 4 - with sites") +
...
plot_mtb_relbipc(mod_week4$res, boot = mod_week4$boot) + 
  geom_vline(xintercept = c(1.5, 4.5, 8.5), linetype = "dashed", col = "gray50") +
  scale_color_manual(values = block_colors) + 
  ggtitle("Initial phase - without site", subtitle = "post-MTZ to week 4") + 
  
  plot_mtb_relbipc(mod_res4_sites$res, boot = mod_res4_sites$boot) + 
  geom_vline(xintercept = c(1.5, 4.5, 8.5), linetype = "dashed", col = "gray50") +
  scale_color_manual(values = block_colors) + 
  ggtitle("Initial phase - with site", subtitle = "post-MTZ to week 4") +
...
#### From week 4 to week 8 and 12

```{r}

model_par$target_visit <- 4

model_par$input_blocks <- mod_week8_and_12$inputs_raw |> names()
model_par$input_blocks <- model_par$input_blocks |> str_replace("Demographics", "Demographics with site")
model_par$model_name <- mod_week8_and_12$model_name |> str_c(" site included in demographics")

mod_res8_and_12_sites <- perform_MBPLSDA_successive(model_par = model_par, mbplsda_par = mbplsda_par, IO_par = IO_par)
...
coefs <-
  bind_rows(
    get_mtb_coef(mod_res8_and_12_sites$res, boot = mod_res8_and_12_sites$boot) |> 
      mutate(model = "With site", block = block |> str_remove(" with site")),
    get_mtb_coef(mod_week8_and_12$res, boot = mod_week8_and_12$boot) |> 
      mutate(model = "Original")
  ) |> 
...
compare_coefs(coefs) +
  ggtitle("Continuation phase (Week 4 to 8 and 12)")
...
plot_mtb_cov(mod_week8_and_12$res, scale_axes = "fixed") + 
  scale_color_manual(values = block_colors) + 
  ggtitle("Week 4 to week 8 and 12 - without sites") + 
plot_mtb_cov(mod_res8_and_12_sites$res, scale_axes = "fixed") + 
  scale_color_manual(values = block_colors) + 
  ggtitle("Week 4 to week 8 and 12 - with sites") +
  plot_layout(guides = "collect", nrow = 2)
...
plot_mtb_relbipc(mod_week8_and_12$res, boot = mod_week8_and_12$boot) + 
  geom_vline(xintercept = c(1.5, 4.5, 8.5), linetype = "dashed", col = "gray50") +
  scale_color_manual(values = block_colors) + 
  ggtitle("Continuation phase - without site", subtitle = "week 4 to week 8 and 12") + 
  
  plot_mtb_relbipc(mod_res8_and_12_sites$res, boot = mod_res8_and_12_sites$boot) + 
  geom_vline(xintercept = c(1.5, 4.5, 8.5), linetype = "dashed", col = "gray50") +
  scale_color_manual(values = block_colors) + 
  ggtitle("Continuation phase - with site", subtitle = "week 4 to week 8 and 12") +
...
model_par$input_blocks <- mod_week24$inputs_raw |> names()
model_par$input_blocks <- model_par$input_blocks |> str_replace("Demographics", "Demographics with site")
model_par$model_name <- mod_week24$model_name |> str_c(" site included in demographics")

mod_res24_sites <- perform_MBPLSDA_successive(model_par = model_par, mbplsda_par = mbplsda_par, IO_par = IO_par)
...
coefs <-
  bind_rows(
    get_mtb_coef(mod_res24_sites$res, boot = mod_res24_sites$boot) |> 
      mutate(model = "With site", block = block |> str_remove(" with site")),
    get_mtb_coef(mod_week24$res, boot = mod_week24$boot) |> 
      mutate(model = "Original")
  ) |> 
...
compare_coefs(coefs) +
  ggtitle("Follow-up phase (Week 12 to 24)")
...
plot_mtb_cov(mod_week24$res, scale_axes = "fixed") + 
  scale_color_manual(values = block_colors) + 
  ggtitle("Week 12 to 24 - without sites") + 
plot_mtb_cov(mod_res24_sites$res, scale_axes = "fixed") + 
  scale_color_manual(values = block_colors) + 
  ggtitle("Week 12 to 24 - with sites") +
  plot_layout(guides = "collect", nrow = 2)
...
plot_mtb_relbipc(mod_week24$res, boot = mod_week24$boot) + 
  geom_vline(xintercept = c(1.5, 4.5, 8.5), linetype = "dashed", col = "gray50") +
  scale_color_manual(values = block_colors) + 
  ggtitle("Follow-up phase - without site", subtitle = "week 12 to week 24") +
  
  plot_mtb_relbipc(mod_res24_sites$res, boot = mod_res24_sites$boot) + 
  geom_vline(xintercept = c(1.5, 4.5, 8.5), linetype = "dashed", col = "gray50") +
  scale_color_manual(values = block_colors) + 
  ggtitle("Follow-up phase - with site", subtitle = "week 12 to week 24") +
...
clin |> 
  filter(!is.na(ARM)) |> 
  select(USUBJID, SITENAME, RACEGR2) |>
  distinct() |>
  dplyr::count(SITENAME, RACEGR2) |> 
  group_by(SITENAME) |> 
  mutate(
    perc = (n / sum(n) * 100),
    value = str_c(n, " (", perc |> round() ,"%)")
  ) |> 
  ungroup() |> 
  arrange(RACEGR2, -perc) |>
  select(-n, -perc) |> 
  mutate(SITENAME = SITENAME |> fct_inorder()) |>
  pivot_wider(names_from = RACEGR2, values_from = value, values_fill = "0 (0%)") |>
  arrange(SITENAME) |> 
  gt(caption = "N (%) of participants by self-declared race at each study site") |> 
  cols_label(SITENAME = "Study site")

```

#### Explanatory power of sites
...
tmp <- 
    bind_rows(
      mod_week4$cv$summary |> filter((dim == opt_ncomp) | is.na(dim), metric == sel_metric) |> 
        mutate(Model = "Original", Phase = "Initial phase"),
      mod_res4_sites$cv$summary |> filter((dim == opt_ncomp) | is.na(dim), metric == sel_metric) |> 
        mutate(Model = "With site", Phase = "Initial phase"),
      mod_week8_and_12$cv$summary |> filter((dim == opt_ncomp) | is.na(dim), metric == sel_metric) |>
        mutate(Model = "Original", Phase = "Continuation phase"),
      mod_res8_and_12_sites$cv$summary |> filter((dim == opt_ncomp) | is.na(dim), metric == sel_metric) |> 
        mutate(Model = "With site", Phase = "Continuation phase"),
      mod_week24$cv$summary |> filter((dim == opt_ncomp) | is.na(dim), metric == sel_metric) |> 
        mutate(Model = "Original", Phase = "Follow-up phase"),
      mod_res24_sites$cv$summary |> filter((dim == opt_ncomp) | is.na(dim), metric == sel_metric) |> 
        mutate(Model = "With site", Phase = "Follow-up phase")
    ) |> 
    mutate(Model = Model |> fct_inorder(), Phase = Phase |> fct_inorder()) 
  
  ylab <- str_c(sel_metric, "\n(mean & 95%CI)")
  if (sel_metric == "average F1 score") ylab <- str_c("Ave. F1 (mean & 95%CI)")
...
tmp |> 
  filter(set %in% c("calibration", "validation")) |>
  ggplot() +
  aes(x = Model, y = mean, col = set) +
  facet_grid(. ~ Phase) +
  geom_errorbar(aes(ymin = `95%CI_lo`, ymax = `95%CI_hi`), width = 0.2) +
  geom_point() +
  geom_text(aes(label = set |> str_sub(1,1)), hjust = 0, vjust = 0, nudge_x = 0.2) +
...
```{r}
model_par$input_blocks <- NULL
model_par$model_name <- NULL
```
...
sample_colors <- 
  mod_week4$output_fct |> 
  str_replace("L. crispatus", "*L. crispatus*") |>
  str_replace("Lactobacillus", "*Lactobacillus*") |> 
  factor(
    levels = c(
      "≥ 50% *L. crispatus*",
...
g_biplot <- 
  plot_mtb_biplot(
    res = mod_week4$res, xaxis = 1, yaxis = 2,  scale_axes = "not eig",
    VIP = TRUE, boot = mod_week4$boot, var_text_size = 4,
    samples_color = sample_colors, 
    sample_color_scale = 
...
selected_cytokines_df <- 
  df |> 
  filter(
    ARM == "LACTIN-V", 
    cytokine %in% c("MIG", "IL-1b"),
    !is.na(week4_status)
  )
...
g_cytokines <- 
  ...
  arrange(AVISITN) |> 
  ggplot() +
  aes(x = week4_status, y = transformed_abundances) +
  facet_grid(cytokine ~ str_c(AVISITN |> get_visit_labels(), "\nconcentrations") |> fct_inorder()) +
  geom_boxplot(
    aes(col = week4_status, fill = week4_status), 
...
t_test_res <- 
  rstatix::t_test(
    data = selected_cytokines_df |> group_by(AVISITN, cytokine) |> filter(AVISITN != 1), 
    formula = transformed_abundances ~ week4_status, 
    ref.group = "≥ 50% L. crispatus",
    p.adjust.method = "BH", var.equal = TRUE
  ) |> 
  rstatix::add_xy_position(x = "week4_status") 

g_cytokines +
  ggpubr::stat_pvalue_manual(t_test_res, label = "p.adj.signif", tip.length = 0.01) +
  labs(
    caption = "Statistical significance was determined by t-test (reference group: ≥ 50% L. crispatus)\nwith Benjamini-Hochberg correction by visit and cytokine."
  )
 
 
 # The results of the MB-PLS-DA suggest that levels of MIG and IL-1b at baseline (pre-MTZ) are associated with subsequent colonization by L. crispatus at week 4. 
 # So, we test if the levels of these cytokines at baseline (pre-MTZ) differ between participants who ended up colonizing by L. crispatus at week 4 versus those who did not.
 # Since we only show these two cytokines, we correct for multiple testing across the two cytokines using the Bonferroni method to control for type 1.
 # Because there are more than ~40 individuals in each group, we can safely use a t-test and assume equal variance.

t_test_res <- 
  rstatix::t_test(
    data = 
      selected_cytokines_df |> 
      group_by(AVISITN, cytokine) |> 
      filter(AVISITN == 0) |> 
      mutate(
        week4_status = 
          ifelse(week4_status == "≥ 50% L. crispatus", 
                 "≥ 50% L. crispatus", 
                 "< 50% L. crispatus")
      ), 
    formula = transformed_abundances ~ week4_status, 
  ...
  mutate(
    xmax = 2.5,
    y.position = y.position * 1.1
    )
 
# add the segment manually to display test between L. crisp and non-L. crisp dominated at week 4
g_cytokines <- 
  g_cytokines +
  geom_segment(data = t_test_res, aes(x = 1.7, xend = 3.3, y = y.position * 0.9), linewidth = 0.4, col = "gray") + 
  ggpubr::stat_pvalue_manual(t_test_res, label = "p.adj.signif", tip.length = 0.02, bracket.size = 0.4) 
...
fig_6 <- 
  ...
  plot_layout(ncol = 2, heights = c(1, 2), width = c(4.5,3), design = design, guides = "collect") +
...
load(str_c(fig_out_dir(), "supplementary_ggplots/g_rates_LC_by_race.Rdata"), verbose = TRUE)
load(str_c(fig_out_dir(), "supplementary_ggplots/g_rate_diff_LC_by_race.Rdata"), verbose = TRUE)
...
fig_6 <- 
  free(g_combined_relbicp) +
  g_biplot + inset_element(g_scree, left = 0.7, bottom = 0.8, right = 1, top = 1, align_to = "plot") +
  free(g_cytokines) + guides(fill = "none", col = "none") +
  free(g_rates_LC_by_race) + g_rate_diff_LC_by_race + guides(col = "none") +
  plot_layout(ncol = 2, heights = c(1, 2, 1), width = c(4.5,3), design = design) + # , guides = "collect"
  plot_annotation(tag_levels = "A")
...
#| cache: false
set.seed(2)
ggsave(fig_6, filename = str_c(fig_out_dir(), "Fig6.pdf"), width = 13, height = 12, device = cairo_pdf)
...
save(g_6A, file = str_c(fig_out_dir(), "supplementary_ggplots/S6A.Rdata"))
save(g_6B, file = str_c(fig_out_dir(), "supplementary_ggplots/S6B.Rdata"))
save(g_6C, file = str_c(fig_out_dir(), "supplementary_ggplots/S6C.Rdata"))
...
g_S7A <- g
save(g_S7A, file = str_c(fig_out_dir(), "supplementary_ggplots/S7A.Rdata"))
...
g_S8 <- g
save(g_S8, file = str_c(fig_out_dir(), "supplementary_ggplots/S8.Rdata"))
...
g_S9 <- g
save(g_S9, file = str_c(fig_out_dir(), "supplementary_ggplots/S9.Rdata"))
...
## Supplementary tables of predictors by arm and outcomes

### Outcomes

```{r}
target_visits <- c(4, 7) # week 12 and 24
...
Y_rBV <- 
  ...
  mutate(outcome_var = "rBV", outcome = rBV) |> 
  select(USUBJID, week, outcome_var, outcome)
  # pivot_wider(
...
Y_Lc <- 
  ...
  mutate(outcome_var = "≥ 50% L.c.", outcome = Lc) |>
  select(USUBJID, week, outcome_var, outcome) |>
  # pivot_wider(
...
Y <- bind_rows(Y_Lc, Y_rBV) |> arrange(USUBJID, outcome_var, week)
rm(Y_Lc, Y_rBV)
...
baseline <- 
  ...
  select(
    USUBJID,
    AGE, EDULVL, RACEGR2, N_PAST_BV
  ) |> 
  arrange(USUBJID) |> 
  distinct()

block <- "Demographics/clinical history"

var_desc <- 
  bind_rows(
    tibble(var = "AGE", desc = "Age (years)", block = block),
    tibble(var = "EDULVL", desc = "Education level", block = block),
    tibble(var = "RACEGR2", desc = "Self-declared race", block = block),
    tibble(var = "N_PAST_BV", desc = "N past BV", block = block)
  )
...
pre_MTZ_vag_env <- 
  get_baseline_environment(
    mae = mae, PID = baseline$USUBJID, target_visits = 2
    )$data


block <- "pre-MTZ Vag. env."

var_desc <- 
  bind_rows(
    var_desc, 
    tibble(var = "α diversity", desc = "α diversity", block = block),
    tibble(var = "pH", desc = "pH", block = block)
  )

pre_MTZ_vag_env <- 
  pre_MTZ_vag_env |> 
  rownames_to_column("USUBJID") |> 
  mutate(USUBJID = USUBJID |> str_remove("_[0-9]+$"))
...
pre_MTZ_mb <- 
  get_baseline_microbiota(
    mae = mae, PID = baseline$USUBJID, target_visits = 2, microbiota_assay = NULL
    )$data 


block <- "pre-MTZ microbiota"

var_desc <- 
  bind_rows(
    var_desc, 
    tibble(var = colnames(pre_MTZ_mb), desc = colnames(pre_MTZ_mb), block = block)
  )
...
pre_MTZ_cytokines <- 
  get_baseline_cytokines(
    mae = mae, PID = baseline$USUBJID, target_visits = 2
    )$data


block <- "pre-MTZ cytokines"

var_desc <- 
  bind_rows(
    var_desc, 
    tibble(var = colnames(pre_MTZ_cytokines), desc = colnames(pre_MTZ_cytokines), block = block)
  )
...
post_MTZ_cat <- 
  ...
  dplyr::rename(post_MTZ_cat = value) 


block <- "post-MTZ microbiota category"

var_desc <- 
  bind_rows(
    var_desc, 
    tibble(var = "post_MTZ_cat", desc = "post-MTZ microbiota category", block = block)
  )
...
post_MTZ_vag_env <- 
  get_prev_visit_environment(
    mae = mae, PID = baseline$USUBJID, target_visits = 2
  )$data


block <- "post-MTZ Vag. env."

var_desc <- 
  bind_rows(
    var_desc, 
    tibble(var = "α diversity (r)", desc = "α diversity (residuals)", block = block),
    tibble(var = "pH (r)", desc = "pH (residuals)", block = block),
    tibble(var = "log10(bact. load)", desc = "log10(bact. load)", block = block)
  )
...
behavior <- 
  ...
  as.data.frame() |>
  as_tibble() |>
  filter(ARM %in% arms, AVISITN > 1, PIPV) |>
  select(
    USUBJID, AVISITN,
    N_SINCE_LPIPV_UNPROT_SEX, N_SINCE_LPIPV_PROT_SEX, N_NEW_PARTNERS
  ) |> 
  arrange(USUBJID, AVISITN) |>
  distinct() |> 
  group_by(USUBJID) |>
  mutate(
    any_unprot_sex = ifelse(cumsum(N_SINCE_LPIPV_UNPROT_SEX > 0) > 0, "No sex", "1+ sex") |> factor(levels = c("No sex", "1+ sex")),
    any_prot_sex = ifelse(cumsum(N_SINCE_LPIPV_PROT_SEX > 0) > 0, "No sex", "1+ sex") |> factor(levels = c("No sex", "1+ sex")),
    total_new_partners = cumsum(N_NEW_PARTNERS)
  ) |> 
  ungroup() |> 
  filter(AVISITN %in% target_visits) |>
  mutate(week = ifelse(AVISITN == 4, 12, 24)) |>
  select(USUBJID, week, starts_with("any_"), total_new_partners)
  


block <- "Sexual behavior"

var_desc <- 
  bind_rows(
    var_desc, 
    tibble(var = "any_unprot_sex", desc = "Self-reported sex without condom", block = block),
    tibble(var = "any_prot_sex", desc = "Self-reported sex with condom", block = block),
    tibble(var = "total_new_partners", desc = "N new partners", block = block)
  )
...
perturbations <- 
  ...
  select(
    USUBJID, AVISITN,
    N_SINCE_LPIPV_DOUCH, N_BLEED_LPIPV
  ) |> 
  arrange(USUBJID, AVISITN) |>
  distinct() |> 
  group_by(USUBJID) |>
  mutate(
    n_douching = N_SINCE_LPIPV_DOUCH,
    n_bleeding = N_BLEED_LPIPV
  ) |> 
  ungroup() |> 
  filter(AVISITN %in% target_visits) |>
  mutate(week = ifelse(AVISITN == 4, 12, 24)) |>
  select(USUBJID, week, n_douching, n_bleeding)
...
var_desc <- 
  bind_rows(
    var_desc, 
    tibble(var = "n_douching", desc = "Nb of days with self-reported douching since last visit", block = block),
    tibble(var = "n_bleeding", desc = "Nb of days with self-reported bleeding since last visit", block = block)
  )
...
table_data <- 
  Y |> 
  left_join(baseline, by = join_by(USUBJID)) |> 
  left_join(pre_MTZ_vag_env, by = join_by(USUBJID)) |>
  left_join(pre_MTZ_mb, by = join_by(USUBJID)) |>
  left_join(pre_MTZ_cytokines, by = join_by(USUBJID)) |>
  left_join(post_MTZ_cat, by = join_by(USUBJID)) |>
  left_join(post_MTZ_vag_env, by = join_by(USUBJID)) |>
  left_join(behavior, by = join_by(USUBJID, week)) |>
  left_join(perturbations, by = join_by(USUBJID, week)) |>
  arrange(ARM, USUBJID) |> 
  mutate(
    outcome_num = outcome,
    outcome = 
      case_when(
        (outcome_var == "≥ 50% L.c.") & (outcome == 1) ~ "≥ 50% L.c.",
        (outcome_var == "≥ 50% L.c.") & (outcome == 0) ~ "< 50% L.c.",
        (outcome_var == "rBV") & (outcome == 1) ~ "rBV",
        (outcome_var == "rBV") & (outcome == 0) ~ "No rBV",
      )
  )
...
gt_rBV_w12 <- make_stratified_table(table_data = table_data, target_week = 12, outcome_of_interest = "rBV", var_desc = var_desc)
gt_rBV_w24 <- make_stratified_table(table_data = table_data, target_week = 24, outcome_of_interest = "rBV", var_desc = var_desc)
gt_Lcr_w12 <- make_stratified_table(table_data = table_data, target_week = 12, outcome_of_interest = "≥ 50% L.c.", var_desc = var_desc)
gt_Lcr_w24 <- make_stratified_table(table_data = table_data, target_week = 24, outcome_of_interest = "≥ 50% L.c.", var_desc = var_desc)
...
# library(openxlsx)
# wb <- createWorkbook()
# addWorksheet(wb, "rBV_week12", gridLines = F)
# gt_rBV_w12 |> gt_to_xlsx(wb, "rBV_week12")
# addWorksheet(wb, "rBV_week24", gridLines = F)
# gt_rBV_w24 |> gt_to_xlsx(wb, "rBV_week24")
# addWorksheet(wb, "Lc_week12", gridLines = F)
# gt_Lcr_w12 |> gt_to_xlsx(wb, "Lc_week12")
# addWorksheet(wb, "Lc_week24", gridLines = F)
# gt_Lcr_w24 |> gt_to_xlsx(wb, "Lc_week24")
# saveWorkbook(wb, str_c(fig_out_dir(), "spreadsheets/Predictors_by_arm_and_outcomes.xlsx"))
...
gtsave(
  gt_rBV_w12, 
  str_c(fig_out_dir(),
        "spreadsheets/Predictors_by_arm_and_rBV_by_week12.html")
)

gtsave(
  gt_rBV_w24, 
  str_c(fig_out_dir(),
        "spreadsheets/Predictors_by_arm_and_rBV_by_week24.html")
)
...
gtsave(
  gt_Lcr_w12, 
  str_c(fig_out_dir(),
        "spreadsheets/Predictors_by_arm_and_Lc_at_week12.html")
)
...
gtsave(
  gt_Lcr_w24, 
  str_c(fig_out_dir(),
        "spreadsheets/Predictors_by_arm_and_Lc_at_week24.html")
)
...
gtsave(
  gt_rBV_w12, 
  str_c(fig_out_dir(),
        "spreadsheets/Predictors_by_arm_and_rBV_by_week12.pdf")
)

gtsave(
  gt_rBV_w12, 
  str_c(fig_out_dir(),
        "spreadsheets/Predictors_by_arm_and_rBV_by_week12.docx")
)
