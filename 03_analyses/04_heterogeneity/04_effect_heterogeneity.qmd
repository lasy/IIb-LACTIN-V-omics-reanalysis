...
topics_long <- 
  ...
  mutate(
    topic = 
      topic |> fct_inorder() |> 
      fct_relevel("Ca. L. v. (BVAB1) topic", after = Inf),
    topic_print =
      case_when(
        str_detect(topic, "BVAB1") ~ "*Ca.* L. v. (BVAB1) topic",
        str_detect(topic, "topic") ~ str_c("*", topic |> str_remove(" topic"), "*"),
        str_detect(topic, "Other") ~ str_c("Other *L.*"),
        TRUE ~ str_c("*",topic,"*") 
      ) |> factor() |> fct_reorder(topic |> as.numeric())
    ) 
...
outcomes_ordered_filtered <- 
 AYV |> 
  ...
  mutate(
    Outcome = ifelse(str_detect(outcome, "Lc"), "≥50%<br>*L.c.*", "rBV"),
    week = str_c(
...
g_observed_topics <- 
  ...
  scale_fill_manual(
    ...
    labels = topics_long$topic_print |> levels(),
...
g_observed_topics_incl <- 
  topics_long_ordered_filtered |> 
  filter(most_prev_genus != "Sneathia", most_prev_genus != "Fannyhessea", most_prev_genus != "Bifidobacterium") |>
...
g_observed_outcomes_incl <-
  ...
  mutate(
    mpg = 
      str_c("*", most_prev_genus, "*") |> 
      str_replace("actobacillus", ".") |> 
      str_replace("\\*Ca. Lachn. \\(BVAB1\\)\\*", "BVAB1") |>  
      fct_inorder()# ,
    # Outcome = Outcome |> str_replace("L\\.c\\.", "*L.c.*"),
    # Outcome_facet = Outcome |> str_replace("\n", "<br>"),
    ) |>
  ggplot() +
  aes(x = USUBJID, y = week |> fct_rev(), fill = Outcome, alpha = value) +
  geom_tile() +
...
differences <- 
    map(
      outcomes, 
      ~ get_RD(AYV, outcome = .x, covariate = "most_prev_genus", min_n_each_arm = 2) |> 
        mutate(outcome = .x)
    ) |> 
    bind_rows() 

n_per_arm_strata_and_outcome_long <- 
    map(
      outcomes, 
      .f = function(outcome, AYV, covariate, min_n_in_each_arm){
        AYV |> 
          select(ARM, all_of(c(outcome, covariate))) |>
          mutate(outcome = outcome, outcome_value = .data[[outcome]]) |>
          drop_na() |> 
          ...
          ungroup() |> 
          filter(n_arm == 2, min_n >= min_n_in_each_arm) |> 
          group_by(outcome, .data[[covariate]], ARM, outcome_value) |> 
          summarize(n = n(), .groups = "drop") |> 
          arrange(, outcome, .data[[covariate]], ARM, outcome_value)
      },
      AYV = AYV, covariate = "most_prev_genus", min_n_in_each_arm = 2
    ) |> 
  bind_rows()

n_per_arm_strata_and_outcome <- 
  n_per_arm_strata_and_outcome_long |> 
  pivot_wider(names_from = c(ARM, outcome_value), values_from = n, values_fill = 0) |> 
  select(outcome, most_prev_genus, starts_with("LACTIN-V_"), starts_with("Placebo_")) |> 
  mutate(
    `n_LACTIN-V` = rowSums(across(starts_with("LACTIN-V_"))),
    `n_Placebo` = rowSums(across(starts_with("Placebo"))),
    n = `n_LACTIN-V` + `n_Placebo`,
    `p_LACTIN-V` = `LACTIN-V_1` / (`LACTIN-V_0` + `LACTIN-V_1`),
    `p_Placebo` = `Placebo_1` / (`Placebo_0` + `Placebo_1`),
    RD = `p_LACTIN-V` - `p_Placebo`,
    RD_SE = 
      sqrt(
        `p_LACTIN-V` * (1 - `p_LACTIN-V`) / `n_LACTIN-V` + 
          `p_Placebo` * (1 - `p_Placebo`) / `n_Placebo`
      )
  )
...
n_per_arm_strata_and_outcome |> 
  group_by(outcome) |> 
  summarize(
    pval = biostatUZH::gailSimon(RD, RD_SE)[1],
    stat = biostatUZH::gailSimon(RD, RD_SE)[2]
  )

# no evidence of qualitative heterogeneity (all p >>> 0.05)
...
g_S5A <- g[[1]]
g_S5B <- g[[2]]
save(g_S5A, file = str_c(fig_out_dir(), "supplementary_ggplots/S5A.Rdata"))
save(g_S5B, file = str_c(fig_out_dir(), "supplementary_ggplots/S5B.Rdata"))
...
AYV <- 
  ...
  mutate(
    most_prev_genus_2 =
      str_c("*", most_prev_genus, "*") |> 
      str_replace(" Lachn\\. \\(BVAB1\\)\\*", "* Lachn.<br>(BVAB1)") |> 
      factor() |> fct_reorder(most_prev_genus |> as.numeric())
  )
```
...
g_strat_LC <- 
  (
      (
        plot_stratified_risks(AYV, outcomes[1:2], strata = "most_prev_genus_2", min_n_in_each_arm = 2) +
          guides(col = "none") 
        ) /
        (
        plot_stratified_differences(AYV, outcomes[1:2], strata = "most_prev_genus_2", min_n_in_each_arm = 2) +
         guides(col = "none") +
          geom_label(
...
g_strat_rBV <- 
  (
      (
        plot_stratified_risks(AYV, outcomes[3:4], strata = "most_prev_genus_2", min_n_in_each_arm = 2) 
        ) /
        (
        plot_stratified_differences(AYV, outcomes[3:4], strata = "most_prev_genus_2", min_n_in_each_arm = 2) +
          geom_label(
...
(
  ...
  ( 
    (g_strat_LC | g_strat_rBV) + plot_layout(ncol = 2) & theme(axis.text.x = ggtext::element_markdown(angle = 45, hjust = 1))
  ) +
...
AYV <- AYV |> 
  arrange(Race) |> 
  mutate(Race2 = Race |> str_replace("Black/African American", "Black/Afr. Am.") |> fct_inorder())

g_rates_LC_by_race <- plot_stratified_risks(AYV, outcomes[1:2], strata = "Race2", min_n_in_each_arm = 2)
g_rate_diff_LC_by_race <- plot_stratified_differences(AYV, outcomes[1:2], strata = "Race2", min_n_in_each_arm = 2) 

save(g_rates_LC_by_race, file = str_c(fig_out_dir(), "supplementary_ggplots/g_rates_LC_by_race.Rdata")) 
save(g_rate_diff_LC_by_race, file = str_c(fig_out_dir(), "supplementary_ggplots/g_rate_diff_LC_by_race.Rdata")) 
