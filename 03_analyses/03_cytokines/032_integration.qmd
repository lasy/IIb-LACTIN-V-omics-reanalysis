...
g <- 
  rv_by_visits |>
  mutate(rv_obs = rv$obs) |> 
  ggplot(aes(x = visit, y = RV, size = -log10(adj_pval), col = adj_pval < 0.05)) +
  geom_hline(aes(yintercept = rv_obs |> unique()), linetype = 2) +
  geom_point() +
...
#| cache: false
g_S4C <- g
save(g_S4C, file = str_c(fig_out_dir(), "supplementary_ggplots/S4C.Rdata"))
...
#| eval: false
tibble(
  label = c("AB",  "A\nB",  expression(italic("A")~"B")) # "A*B*","A<br>B",
) |> 
  mutate(x = row_number(), y = 1) |> 
  ggplot() +
  aes(x = x, y = y, label = label) +
  # geom_text(parse = TRUE) +
  ggrepel::geom_text_repel(parse = TRUE)
...
tibble(
  x = 1:5,
  y = 1:5,
  label = c("italic('Genus species')", 
            "'Normal text'",
            "italic('Italic text')",
            "paste('Mix ', italic('italic'), ' and normal')",
            "italic('All italic')")
) |> 
  ggplot() +
  aes(x, y, label = label) +
...
cors <- 
  ...
  mutate(
    feature_print = 
      case_when(
        str_detect(feature, "^L\\. .*") ~ str_c("italic('",feature,"')"),
        str_detect(feature, "BVAB1") ~ "paste(italic('Ca.'), ' L. v. (BVAB1) topic')",
        str_detect(feature, "Other") ~ "paste('Other ',italic('L.'))",
        str_detect(feature, "topic$") ~ str_c("paste(italic('",feature |> str_remove(" topic"),"'), ' topic')"),
        TRUE ~ str_c("'",feature,"'")
      )
  )
...
g_cor_circle_L1_12 <- 
  ...
  geom_text_repel(
    aes(label = feature_print), parse = TRUE,
...
g_S4F <- g[[2]]
g_S4G <- g[[3]]
save(g_S4E, g_S4F, g_S4G, file = str_c(fig_out_dir(), "supplementary_ggplots/S4EG.Rdata"))
