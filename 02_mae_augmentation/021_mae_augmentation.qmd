...
This suggests that there might be 8 or 9 true sub-communities.
...
plot_beta(aligned_topics_transport, models = c(5, 6, 7, 8, 9)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

```

From these plots, it looks like K = 8 might be a good compromise between dimension reduction, representation accuracy, and minimizing the number of spurious topics.


We note that at none of these resolutions (K = 5, 6, 7, 8, 9) do we find any topic similar to the subCSTs IV-C(0-4).
...
The main "issue" is with the topic dominated by *L. jensenii*:
...
beta_long <- get_beta_long(mae, assayname = "topics_16S_8")
beta_long |> 
  filter(topic_subcst_matching_label %in% c("I","III","V") | str_detect(topic_label, "L. jensenii/mulieres")) |> 
  filter(prop > 1/500) |>
  arrange(topic, -prop) |>
...
g <- 
  ...
  scale_color_manual(values = get_topic_colors(c("III","I", "V", rep("VI", 8)))) +
  guides(col = "none")
...
g_S2C <- g
save(g_S2C, file = str_c(fig_out_dir(), "supplementary_ggplots/S2C.Rdata"))
...
Lacto_topics_beta["Lactobacillus jensenii/mulieres","V"] <- 1

all_Lacto_species <- str_subset(colnames(props), "Lactobacillus")
other_Lacto_species <- 
setdiff(
  all_Lacto_species, 
  str_c("Lactobacillus ", c("crispatus","iners","jensenii/mulieres"))
  )
...
Lacto_topics_gamma <- 
  cbind(
    props[, str_c("Lactobacillus ", c("crispatus","iners","jensenii/mulieres"))], 
...
g_S2A <- g[[1]]
g_S2B <- g[[2]]
save(g_S2A, file = str_c(fig_out_dir(), "supplementary_ggplots/S2A.Rdata"))
save(g_S2B, file = str_c(fig_out_dir(), "supplementary_ggplots/S2B.Rdata"))
...
estimated_sample_composition <- 
  t(SummarizedExperiment::assay(mae[["c_topics_16S_8"]])) %*% mae[["c_topics_16S_8"]]@metadata$beta

observed_sample_composition <- 
  t(SummarizedExperiment::assay(mae[["tax_16S_p"]]))[rownames(estimated_sample_composition), colnames(estimated_sample_composition)] 
...
pure_lacto_topics <- 
  c(
    ...
    "Lactobacillus iners",
    "Lactobacillus jensenii/mulieres"
  )
