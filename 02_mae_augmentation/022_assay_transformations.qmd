...
g <- 
  cytok_corr  |> 
  as.data.frame() |>
  rownames_to_column("cytokine_1") |>
  mutate(cytokine_1 = cytokine_1 |> fct_inorder()) |> 
  pivot_longer(
    cols = -cytokine_1,
    names_to = "cytokine_2",
    values_to = "cor"
  ) |>
  mutate(cytokine_2 = cytokine_2 |> fct_inorder()) |> 
  filter(cor != 1) |>
  ggplot() +
  aes(x = cytokine_1, y = cytokine_2, fill = cor) +
  geom_tile() +
  geom_text(aes(label = round(cor, 2)), size = 3) +
  scale_fill_distiller(
    "Pearson\ncorrelation",
    type = "div", palette = "RdBu", limits = c(-1, 1), direction = 1
    ) +
  xlab("") + ylab("") 
...
g_S4B <- g
save(g_S4B, file = str_c(fig_out_dir(), "supplementary_ggplots/S4B.Rdata"))
...
g_S4A <- g
save(g_S4A, file = str_c(fig_out_dir(), "supplementary_ggplots/S4A.Rdata"))
